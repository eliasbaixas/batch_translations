  ...
  module Helpers
    class FormBuilder
      ...
      def globalize_fields_for(locale, *args, &proc)
        ...
        object_name = "#{@object_name}[globalize_attributes][#{@index}]"
        object = @object.globalize.find_by_locale locale.to_s
      ...
      module Translated
        module Callbacks
          def enable_nested_attributes
            accepts_nested_attributes_for :globalize
          end
        end
        module InstanceMethods
          ...
          def init_translations
            I18n.available_locales.reject{|key| key == :root }.each do |locale|
              translation = self.globalize.find_by_locale locale.to_s
              if translation.nil?
                # logger.debug "Building empty translation with locale '#{locale}'"
                globalize.build :locale => locale
                save
              end
            end
          end
